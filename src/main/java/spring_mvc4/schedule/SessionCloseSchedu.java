 ...
 * 定时任务，因容器非spring管理，所以现在只能遗弃，用最底层的timeerTask
 */
public class SessionCloseSchedu {

    private static Log log = LogFactory.getLog(SessionCloseSchedu.class);

    private List<HttpSession> list;
    private Object lock;

    public SessionCloseSchedu() {
    }

    public SessionCloseSchedu(List<HttpSession> list, Object lock) {
        this.list = list;
        this.lock = lock;
    }
        ...
        synchronized (lock) {
            ...
            */
            while (it.hasNext()){
                ...
                /**
                * 如果当前时间-session的最后访问时间>1000*30(30秒)
